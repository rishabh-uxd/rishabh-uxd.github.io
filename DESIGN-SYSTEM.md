...
iterations/70-autopay-how-it-works-new-frame/  **the Automatic payments frame is built and HELD, not shipped**, asked for the same way as 69, a screenshot of the live asset beside a node: *"update this image with: [node-id=2445-49028]"*. `assets/img/pay-bill/autopay-how-it-works.webp` was **not replaced**. The export renames the screen's primary button from **"Set up automatic payments"** to **"Submit payment"**, which is the Confirm payment screen's button label two pairs up in section 05 and is wrong on a screen that is the entry point to enrollment and submits no payment. That was confirmed against Figma's own render of the node rather than the PNG export, so it is the frame saying it; the crop is in `candidate/figma-node-button.png`. Shipping it would have put a wrong call to action on a page meant to show shipped work, which is a content error rather than a rendering artifact, and would have made the figure's alt text false, since it ends "and a Set up automatic payments button". **The export is not one sided**, which is why this is a trade rather than a rejection: it **fixes the clipped Chase disclosure**, the most visibly wrong thing in the screenshot the user pasted, and it makes the button a pill, which would bring this screen into line with the three rebuilt in 66, 67 and 69 and leave only bank accounts and activity on the old shape. The trade is still the wrong way round, because a clipped line of 6px disclosure text is a mock artifact and a wrong button label is a claim about the product. Everything else is identical, Current status Off, the same three numbered steps, the same disclosure sentence, with the content sitting 6px higher (ink row profiles: mean absolute difference 22.0 at zero offset, 16.3 at -6) and the file 76,328 bytes against 80,018. The bezel is 37px at mid height, same as the live asset and every other Pay Bill phone asset, so iteration 66's thinner bezel finding failed to hold for the second time. `proc.py` is the encoder of record and is **ready to run unchanged** the moment the frame says "Set up automatic payments" again: keyed output is already 840 x 1731 with alpha bbox (0, 0, 840, 1731), which this file shares with its pair partner `autopay-confirm-settings.webp`, so shipping it will need no markup change at all. The built asset is kept at `candidate/autopay-how-it-works.webp` with a side by side against the live one. **No asset, markup or attribute in the repo changed**; this entry is the only tracked edit, recording the hold as a decision rather than an oversight.
