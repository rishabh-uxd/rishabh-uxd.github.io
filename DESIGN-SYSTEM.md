...
iterations/57-built-for-the-team/  the Practice band renamed **"Built for the team"** and its standfirst made to reach the divider above it, asked for together: *"The text width should match the divider widht on top. Also, come up with a better name than "Practice" ... documents I have written and tools I have built to uplevel the entire team"*. **The name** goes on the band h2, the nav label on all nine pages, and `practice.html`'s `<title>` and hero label; **`id="practice"` and the `#practice` fragment deliberately do not move**, because they are the target of the nav link on nine pages and of any URL already shared, and the file is still `practice.html`, so renaming them buys nothing a reader can see. "Practice" was a designer's word for a body of work, so a recruiter read these four as more work samples, which is the one thing they are not; "Built for the team" names the audience instead of the artifact. Rejected: "Internal tools" (two of the four are documents), "Team enablement" (HR register, and section 3 rules out the abstract noun), "Documents and tools" (makes the artifact the subject). Nav overflow with the longest label in the pill was measured on `scrollWidth` vs `clientWidth` at both `.nav__inner` and `.nav__links`, not eyeballed. **The width defect was real and was in shipped code**: `--measure-lead-wide: 84ch` was calibrated where the container ran 1170 to 1390, and `--page-max` puts it at about 1690 on a 32 inch monitor, so 84ch's 1210 stopped a third short of the rule. **A measure can never do this job** (it is a fixed box, so it matches at one viewport and falls short at every wider one) and widening it is not available, because span and size are one dial and a compliant 80 character line is about 920px at any viewport for any copy. The fix is **two paragraphs side by side in a two column grid**, one column below 860px: a two column block has no width of its own, so it lands on the divider by construction at every width, and because each paragraph is whole the reader finishes a column before moving right. **Four passes, and the last two are the lesson.** `column-width: 33rem` spans correctly and **introduced a new 1.4.8 failure of 83 characters at 1024** in a single column, because `column-width` is a minimum not a count; `28rem` moved the failure rather than removing it, since the gap is inside the flip arithmetic (`(container - gaps) / n`); the `column-width` version also gave **three columns above 2100, numerically fine and wrong on the page**, two or three lines a column reading as three fragments and breaking "documentation a" / "designer can learn from" across a boundary, rejected on a screenshot at 2560. Then **`column-count: 2` shipped for exactly one commit and the user rejected it at once**, *"Why is this text breaking into two columns?"*, which is the finding worth keeping: **CSS columns flow ONE paragraph across the break**, so at three lines a column the sentence continues at the top of the second column and the reader has to travel back up to finish it. That is a paragraph cut in half, not a paragraph in two columns, and it is the three column complaint again at two. **Do not reach for `column-count` on running prose a reader is meant to read straight through** — it is right for a list or a set of short independent items. The answer was structural rather than typographic, two separate paragraphs in a grid, same span and same line length with nothing to reassemble. 860px and the gap are **borrowed from `.practice-grid`**, and the deck's collapse lives inside `.practice-grid`'s own `max-width: 860px` block rather than in a `min-width` mirror of it, because written as mirrors both applied at exactly 860px and the deck held two columns over one column of cards. **`--measure-lead-wide` is deleted and with it the site's only WCAG 1.4.8 violation**; the worst line on the site is now 77, stacked full width between 744 and 860, with three characters of headroom, and `--page-max`'s 1800 cap is what keeps 2560 and 3840 identical at 72. `--t-lead-wide`'s ramp went `2.4vw` to `2.5vw` to buy that headroom. **The copy was rewritten twice, and the treatment dictates it**: the old 137 character deck would have set as one line per column, and a two paragraph grid needs the two close to the same length or the right column ends a line short, so they are 205 and 207 characters and both set as three lines at 2560. The second paragraph names the pairing the 2x2 grid already draws (the documentation and the tool, then the agent and the playbook), which was a claim only a code comment was making. The rewrite also dropped "the playbook the agent came out of", the last provenance claim of its kind in home page copy.
