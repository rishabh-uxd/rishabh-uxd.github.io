...
iterations/71-pay-bill-final-launch-2024/  **Pay Bill's timeline says the release was phased**, asked for as *"Update timeline to for pay bill to communicate final launch in 2024"* and then scoped a message later with *"in the pay bill project hero I mean"*. The hero rail's Timeline cell went from "March 2022, launched October 2023" to **"March 2022, initial release October 2023, final release 2024"**. Copy only, no asset and no CSS. **Section 03 moved one sentence with it**, and that was not scope creep: it said flatly "Pay Bill launched in October 2023", so leaving it while the rail above named a 2024 release would have put two different endings on one page, with the rail being the one a reader meets first. It now reads "Pay Bill's first release launched in October 2023 and the last of it in 2024", and the sentence after it anchors the September 2024 analysis as "about eleven months after that first release" rather than the bare "eleven months in", because eleven months is measured from October 2023 and a page with two release dates has to say which one the interval hangs off. The phasing was always in the copy: section 03 already called October 2023 "the initial release" twice, in the paragraphs about the slider being cut from it, and the rail was the part that contradicted it. **The line break had to be forced.** At 15px in a rail 285 to 363px wide the value is two lines at 1512, 1280, 1024 and 390 alike, which is fine, but left alone it split a phrase, "... October 2023, final / release 2024"; three `&nbsp;` entities tie "October 2023" and "final release 2024" together so the break lands at the comma, the same device the Team cell already uses for "15&nbsp;engineers". That was caught by reading a capture rather than by measuring, since the measurement said two lines both before and after and said nothing about where they split. **No other page carries a Pay Bill date**, grepped across `index.html`, `about.html` and every case study, so nothing moved with it. **The eleven per-screen captions still say "Launched October 2023" and are left alone on purpose**: which screens shipped in the first release and which in 2024 is not something this repo knows, and a date under a specific screen on the strength of a guess is worse than a loose one. No month is claimed for 2024 because none was given, and 2024 is the user's own statement of when the project finished. Ports 23753, 23759 and 23761, `/tmp/audit57.py` over 8 pages and 5 viewports **ALL CLEAN**.
iterations/70-autopay-how-it-works-new-frame/  **the Automatic payments screen, held for one wrong word and then shipped**, node `2445:49028`, asked for the same way as 69, a screenshot of the live asset beside a node: *"update this image with: [node-id=2445-49028]"*, then released two messages later with *"updated the figma so it's correct now"*. It took **two exports of the same node**. The first renamed the screen's primary button from **"Set up automatic payments"** to **"Submit payment"**, which is the Confirm payment screen's button label two pairs up in section 05 and is wrong on a screen that is the entry point to enrollment and submits no payment, so the built asset was **held in `candidate/` and reported instead of published**: it would have put a wrong call to action on a page whose whole claim is that the work shipped, a content error rather than a rendering artifact, and it would have made the figure's alt text false, since the alt ends "and a Set up automatic payments button". That the frame rather than the export said it was confirmed against Figma's own render of the node, crop at `candidate/figma-node-button.png`, which is what made the hold defensible rather than speculative. **The hold was a trade, not a walkover**, worth recording because the shape will recur: the held export also **fixed the clipped Chase disclosure**, the most visibly wrong thing in the screenshot the user pasted. A clipped line of 6px disclosure text is a mock artifact and a wrong button label is a claim about the product, so it is an order, but it is an order rather than a one sided rejection. The user fixed the frame, so this iteration keeps both exports, the corrected one in `source/` and the first in `source-first-export/`; thresholded at 60 **they differ in one box, (438, 2703, 1252, 2767), the button label band and nothing else**, and the button was cropped from the new export and read back: "Set up automatic payments". `assets/img/pay-bill/autopay-how-it-works.webp` was then **replaced**, `proc.py` being the encoder of record for a file that had none, with everything below its docstring a copy of iteration 67's, which is 66's, which is 65's: same keyer, same 505 background-run rows assertion, 840 x 1731, a size this file shares with its pair partner `autopay-confirm-settings.webp` and so cannot change alone. Against the outgoing asset the shipped one carries the **unclipped disclosure**, a **pill** primary button where it was a rounded rectangle, the content **6px higher** (ink row profiles: mean absolute difference 22.0 at zero offset, 16.3 at -6), and one change that is new and Figma side: **the lit tab moved from Account to Home**, cropped from both files at 2x and read rather than inferred, which leaves this screen disagreeing with its pair partner, which still lights Account. Account is the more plausible of the two for a credit card settings screen, so that reads as a regression in the frame; it did not hold the ship because it is about 6px of teal where this renders, no page copy or alt text names the tab bar, and unlike a button label it makes no claim a reader could act on. It joins the slider screen's disagreeing cents on the Figma side list. Every other word is identical, Current status Off, the three numbered steps, the same disclosure sentence, and the bezel is 37px at mid height as in every other Pay Bill phone asset, so **iteration 66's thinner bezel finding failed to hold for the third time**. **The pair partner needed nothing and this was checked** rather than assumed, the question iteration 66 got wrong and 69 got right: `autopay-confirm-settings.webp` has **no filled button**, only a disclosure row and a checked agreement box, so a pill beside it disagrees with nothing, and its bezel is the same 37px. Section 05 now holds pills on four rebuilt screens and rounded rectangles on bank accounts and activity only, none of them adjacent to a pill, and the nodes for those are not known. Output 78,622 bytes against 80,018, quality 88 method 6, alpha bbox (0, 0, 840, 1731), so no `width` or `height` attribute moved and **no markup changed at all**; the export was 1692 x 3487 on the same `#F5F5F5` with the same (63, 61, 59) bezel. The alt text and caption did not change, which is the whole point of the hold: the alt names the current status, the three step explanation and the button, and all three are still true. Ports 23741 and 23747, `/tmp/audit57.py` over 8 pages and 5 viewports **ALL CLEAN**, resolution ratio 1.06 at 1512 and 1.00 at 1280 and 1.21 at 390 for both halves of the pair, the 1.00 being the pair floor iteration 66 recorded and predating this work.
iterations/69-confirm-payment-new-frame/  **the Confirm payment screen rebuilt from the current Figma frame**, node `2445:47493`, asked for by pasting a screenshot of the live asset beside the node: *"update this image with this: [node-id=2445-47493]"*. `assets/img/pay-bill/otp-confirm-payment.webp` is the left half of the second `.shots--pair` in Pay Bill section 05, and `proc.py` here is the encoder of record for a file that had none, since iterations 08 through 11 cover the Pay Bill screens as a set without naming any one file or its crop. Everything below the docstring is a copy of iteration 67's, which is 66's, which is 65's: same keyer, same assertions, 840 x 1731, and that size is a constraint rather than a default because the pair's other half is `otp-payment-submitted.webp` and the two have to match. **Every figure and every word on the screen is identical**, $980.00 selected, Today (April 28, 2022), Chase Checking ***9993, the same 8 PM to 11:59 PM ET paragraph and the same authorization paragraph. Two things moved: the **Submit payment button is a pill** where it was a rounded rectangle, the same change the Continue button made in iteration 66, so this rebuild brings the section's button shapes into agreement rather than splitting them; and the **whole screen sits 17px lower**, about 8 CSS px where it renders, found by sliding the two dark ink row profiles against each other (mean absolute difference 59.1 at zero offset, 17.2 at +17), which is what accounts for the 640 rows a naive comparison flags as changed. **Two of iteration 66's findings were checked here and do not hold**, which is why they are not repeated: the bezel is not thinner, it is 37px at mid height in this file and in every other Pay Bill phone asset including the old generation, and the header search field has not widened, cropped from both files and compared it is the same width in the same place 17px lower. **Payment submitted was left alone on purpose and this is not the iteration 66 situation**, checked before deciding: that pair was one screen shown twice, where two button shapes read as two different screens, and this pair is two different screens of which the confirmation **has no button** for a pill to disagree with, and the same 37px bezel. What it does leave is section 05 holding two generations of the app's button shape, pills on the three rebuilt screens and rounded rectangles on the autopay, bank accounts and activity screens below; none of those is adjacent to a pill so nothing reads as a contradiction, and the nodes for those six are not known. Output 93,038 bytes against 97,146, alpha bbox (0, 0, 840, 1731), so no `width` or `height` attribute moved; the export was 1692 x 3487 on the same `#F5F5F5` with the same (63, 61, 59) bezel and the 505 background-run rows assertion passed, which is the check on the frame geometry. The alt text and caption did not change: the alt names the amount selected, the payment date, the bank account, the 8 PM cutoff and the button, all still true, and a button radius is not a fact a screen reader user needs. One comment was added to `work/pay-bill.html` above the pair recording the rebuild and why the right half was left. Ports 23629 and 23633, `/tmp/audit57.py` over 8 pages and 5 viewports **ALL CLEAN**, resolution ratio 1.06 at 1512 and 1.00 at 1280 and 1.21 at 390 for both halves, the 1.00 being the pair floor iteration 66 recorded and predating this work.
iterations/68-thumbnail-slider-digit/  **the same digit on the home page thumbnail**, asked for as *"Update the slider image in thumbnail as well to reflect 245 at min pay"*. `assets/img/pay-bill/home-card.webp` rebuilt so its right hand phone reads **$245.40** as the minimum payment due instead of $235.40, which puts the home page on the same generation of the screen as the case study after iteration 67 changed it there. `proc.py` here is the encoder of record for this file, superseding iteration 65's, and everything below its docstring is a byte for byte copy of 65's, keyer and assertions and geometry alike, so the only thing that can have changed is the one source file meant to. **The left phone was re-downloaded and had not changed**: node `2445:43500` came back byte for byte identical to the copy iterations 65 and 66 both used, md5 `f8e91aa2af5727c8746b5faf9bd49f4b`, which is the only way to know the user did not adjust the radio screen while fixing the slider and is cheaper than reasoning about it. The right phone is iteration 67's third export of `2445:45969`, already on disk, so no second Figma call was needed. The output is unchanged in every respect but one: 1600 x 1200 RGBA, alpha bbox (236, 47, 1364, 1152), opaque column runs (236, 771) and (828, 1363), and 79,114 bytes before and after. Thresholding the difference between the outgoing and incoming WebPs at 60 gives **80 pixels in an 11 x 15 box at (1253, 695)**, inside the right phone's column run and nowhere near the left one. The threshold is the point: a raw diff on a lossy WebP covers the whole frame and says nothing, a trap this repo has fallen into before. The figure was then cropped from both files at 8x and read back rather than inferred, $235.40 against $245.40. **It is visible, just barely**: the digit is about 5 CSS px tall where the canvas renders 513 CSS px wide at 1512, which is readable on a 2x display and was read off a capture of the live card at both 1512 and 390, so this is a visible correction at the sizes the card is actually seen rather than only bookkeeping. Nothing in the markup or the copy moved. `index.html`'s alt names the custom amount field, the three radio labels, the $550.00 payment and the $13.65 charge and does not quote the minimum payment due, so it is still accurate word for word, and `width` and `height` stay 1600 and 1200. No page text in the repo cites either figure (grep); the only hits are iteration entries here, which are records of past states and correct as written. Resolution ratio 1.56 at 1512 and 2.73 at 390. Ports 23531 and 23537, `/tmp/audit57.py` over 8 pages and 5 viewports **ALL CLEAN**. **Still off, Figma side and untouched by this**: the minimum payment due still does not equal its neighbour, $245.67 on the radio screen against $245.40 on the slider, dollars agreeing and cents not, and both legible on this card as well as in Pay Bill section 05; the current balance disagrees, $1,341.10 against $1,341.00; and the Chase disclosure's second line runs under the tab bar.
iterations/67-slider-minimum-digit/  **one digit on the slider screen**, asked for as *"Update the slider image then: [node-id=2445-45969]"* after iteration 66 reported that the two phones in Pay Bill section 05 disagreed about the minimum payment due. This is the **third** export of that node, and it differs from the second in exactly one place, proven rather than assumed: diffing the two 1692 x 3487 PNGs pixel for pixel gives a bounding box of (1342, 2046) to (1375, 2090), 33 x 44 px at 4x, one digit, **$235.40 becoming $245.40**. That negative result is as useful as the positive one, because it proves the other two open defects were untouched: the current balance still reads $1,341.00 against $1,341.10 on the screen beside it, and the Chase disclosure's second line still runs under the tab bar. And the figure that was fixed **still does not equal its neighbour**, $245.40 here against $245.67 there: the dollars agree and the cents do not, so the gap went from $10.27 to 27 cents rather than to nothing, and at 396 CSS px per phone both cent values are legible one above the other. Read off a screenshot digit by digit rather than inferred from the diff. `proc.py` here is the encoder of record for `slider-select-amount.webp`, superseding iteration 66's for that file only; 66's remains the encoder of record for `otp-select-amount.webp`, and everything below this one's docstring is a copy of 66's with the second job removed so the two halves of the pair cannot drift apart in keying or size. Keying is iteration 65's `load_keyed` for the third time and its assertion that 505 of 3487 rows carry a `#F5F5F5` run passes again, which is the check that Figma has not quietly changed the frame geometry between exports. 840 x 1731 again because the pair has to match, so no `width` or `height` attribute moved and the only edit to `work/pay-bill.html` is one clause naming this iteration. No alt text or caption changed: neither cites the minimum payment due, the caption names $13.65 and the alt "13 dollars 65 cents", and no published copy anywhere cites $235.40 or $245.40 (grep). Thresholding the difference between the outgoing and incoming **shipped** WebPs at 60 gives (666, 1016) to (682, 1038) and 180 pixels, so the digit is the only substantive change to survive the re-encode and the rest of the whole-image difference is lossy noise. `/tmp/audit57.py` over 8 pages and 5 viewports: **ALL CLEAN**.
iterations/66-pay-bill-hero-new-frame/  **the Pay Bill case study hero rebuilt from the Figma frame the user supplied**, node `2445:43500`, asked for as *"Now can you update the hero image with: [node-id=2445-43500]"*. That node is the radio button Select amount screen, which is the left half of the pair iteration 65 built the home page thumbnail from and the screen this hero already showed, so the request is a straight replacement of an old export with the current one rather than a change of subject, and the thumbnail and the hero are now the same generation. The export was **not** re-downloaded: it is byte for byte iteration 65's `source/pb-phone1-4x.png`, copied in so this iteration stands alone, and asking Figma again would only risk a third export differing from the two already shipped. Keying is iteration 65's `load_keyed` unchanged, and its assertion that 505 of 3487 rows carry a `#F5F5F5` run still passes, which doubles as the check that the file is the same one; the one piece of that reasoning which does not transfer is the objection to iteration 60's convex fill, since that was about two phones sharing an image and these hold one each. **The slider screen was rebuilt too, and was not asked for.** Section 05 pairs the shipped screen with `slider-select-amount.webp` under a comment reading "Same screen, same export size, so the comparison is the point", and with only the hero rebuilt that pair rendered two phones at 396 CSS px each with **different Continue button shapes**, a pill against a rounded rectangle, which at that size reads as two different screens; it also put $245.67 next to **$35.40** for the same minimum payment due, an order of magnitude apart on a live page. Rebuilding the slider from the export already on disk fixed both, cost one encode, and introduced nothing, because the one defect in the new slider frame, the Chase disclosure's second line running under the tab bar, is already in the old asset. This was caught by looking at the rendered pair, not at the assets. **840 x 1731 is a constraint, not a default**: the hero is sized by `--case-hero-phone-h` so its intrinsic width is only a resolution ceiling, but the pair's two halves have to match, so both files keep their width even though the 1692px sources could give 940 like `basket-building/bundle-carousel.webp`. The real resolution floor on the page is the pair and it predates this work, 419.2 CSS px per half at 1280 against 840 natural, a ratio of exactly 1.00, no headroom, which is the argument for moving both to 940 together some day. What changed on the screens, measured by profiling dark ink rows rather than guessed: the Continue button became a pill on both, the search bar is slightly wider and the bezel a hair thinner on both, the radio screen sits about 6px higher at this size, and the slider's minimum payment due went from $35.40 to $235.40. Every other figure is untouched, none of them appears in any published copy (grep), and no alt text changed, since all three already named every item on their screen in order and a button radius is not a fact a screen reader user needs. Verified at DPR 2 across 1512, 1280, 860, 744 and 390 with reveals forced and every image eager and complete: `/tmp/audit57.py` over 8 pages and 5 viewports **ALL CLEAN** both before and after the slider rebuild, no document overflow, hero resolution ratio floor 1.38, the pair's two halves the same box at all five widths, and the keyed corners looked at rather than only measured. The light grey slivers at the bezel edges are the phone's own side buttons and were in the old asset too, confirmed by comparing column 0's opaque runs. **Still off and Figma side, none of it introduced here**: the minimum payment due disagrees between the two phones, $245.67 against $235.40, and so does the current balance, $1,341.10 against $1,341.00, and both are now legible side by side where the thumbnail only ever showed them at 171 CSS px per phone, so this is the surface where they matter.
iterations/65-years-off-cards-new-paybill-frames/  **the year is off all five home page project cards**, and the Pay Bill thumbnail rebuilt from new Figma frames, both asked for in one line: *"Remove years from project thumbnails. For Pay Bill thumbnail image specifically, use this updated phone frames: [node-id=2445-43499]"*. This **reverses iteration 62**, which had just moved the same five values from start years to ranges, and it overrules a recommendation I gave in the turn immediately before: I had argued for keeping them, on the grounds that the home page is ordered by strength rather than by date, so a reader with no years assumes reverse chronological and reads KeyBank as the most recent work. That argument is not wrong and it is not the point; the user decided, and the five spans and the four comments that defended them are gone. Where the dates still live: **every case study page carries its own in the Timeline cell of its hero rail**, so nothing became undateable, and iteration 62's entry above holds all five values with the reasoning that derived them if they are ever wanted back. `.project__year` was deleted rule and all, `margin-left: auto; white-space: nowrap`; nothing else in `.project__meta` used `margin-left: auto`, so the index and the tag sit where they always did and the row simply ends after the tag. **If a year ever comes back it does not come back as a third chip**, it goes in the summary sentence where the site's standing rule already puts numbers. One unlooked for gain: the row is a single line at 390 again. `.project__tag` had wrapped "Credit card servicing" to two lines there ever since iteration 36, because the row has no `flex-wrap` and the year was taking the width; the first wrap is now at 360 and the row is 31px at 390 against 43 before. **The Pay Bill thumbnail** is `assets/img/pay-bill/home-card.webp` rebuilt from node `2445:43499` of `H3gOVBp2rmrTas26UAY195`, exported as its two phone children `2445:43500` and `2445:45969` at scale 4, and this iteration's `proc.py` is the encoder of record for an asset that had none before (iterations 08 through 11 do not cover it). The exports arrive **fully opaque on the frame's own `#F5F5F5`**, so the corners have to be keyed back out or the tinted `.project__media` panel shows four grey notches; iteration 14's punched in circle is wrong because the bezel corner is a squircle (a 252px circle would put a 230px run on row 1 and the export puts 242) and iteration 60's row wise convex fill is wrong because it would bridge the 56px gap, so the two phones are keyed **separately** with a plain run inward from each edge while the pixel is `#F5F5F5` plus or minus 3. That run exists on exactly 505 of 3487 rows, 252 at the top and 253 at the bottom, and nowhere else, which is what makes it safe: the bottom corner bands hold 22,657 pure white pixels of screen content that any lightness threshold would have eaten. Geometry is the outgoing asset's own, measured off it before replacement so the thumbnail keeps its size and place: 1600 x 1200, phones 536 wide, 56px gap, 236px side pad. The one departure is vertical, 1105 tall at 47px top pad and 48 bottom rather than 1104 at 48 and 48, because these frames are 423 x 871.517 and forcing 1104 would have squashed them by a quarter of a CSS pixel's worth of aspect. The alt text was rewritten, since the old one was nine words for two screens that have changed. Verified at DPR 2 across 1512, 1280, 1024, 860, 744, 430, 390, 360 and 320: no document overflow at any width, the thumbnail's resolution ratio is 1.10 at its worst (860) and never below 1, and `/tmp/audit57.py` over 8 pages and 5 viewports is **ALL CLEAN**. **The frames were exported twice.** The first export shipped, three defects in it were reported, the user updated the Figma the same day, and the same two nodes were re-exported and the asset rebuilt; the second export is byte-different, same dimensions, same `#F5F5F5` flattening and the same 505 background-run rows, so `proc.py` did not change. Both exports are kept, `source/` being the second and `source-first-export/` the first. The update **fixed the one that mattered**: the left phone carried a `< Back` bar and the right did not, so the two phones disagreed about their own app chrome, which reads worse in a thumbnail than any number on them, and both now open on the search header. **Three are still off, all Figma side and none fixable here**: the minimum payment due still disagrees between the two phones, $245.67 on the radio screen against $235.40 on the slider screen (it was $35.40, so it now looks like a typo rather than a different figure); the current balance is $1,341.10 on one and $1,341.00 on the other; and new in the second export, the second line of the Chase disclosure on the slider screen runs under the bottom tab bar, because that screen frame is 386 x 1123 inside an 836 viewport. All three sit below the legibility threshold at the size the thumbnail renders, which is why the asset ships, but the frames are presumably used at full size elsewhere.
iterations/64-about-portrait-photo/  the about page hero's portrait replaced with a real photograph, asked for as *"For about me page, can you use my hi-res LinkedIn photo?"* and then *"use this photo"* with one attached. **The LinkedIn photo could not be reached by any route**: no `assets/img/about/` existed, the saved profile PDF is text only and a byte scan of all 72,359 bytes found no JPEG stream and no `/Subtype /Image`, this machine has no `pdfimages`, `pdftoppm`, `mutool`, `qpdf` or `gs` and no `fitz`, and a fetch of the public profile returned only the banner. So the user supplied a photograph from the Photos library instead. **The paste handed over a derivative, not the original**, and finding the full resolution version is half the iteration: the pasted file is 768 x 1024, `derivatives/masters/` holds only 360 x 480, `originals/` does not hold the UUID at all, and the best available is `renders/0/..._1_201_a.heic` at **3300 x 4397**. PIL here cannot decode HEIC, so it was transcoded with macOS `sips -s format png` and the transcode is kept in `source/` so `proc.py` runs without the library. **The crop is the decision, not the resize.** As shot it is a full figure at 3:4, head to sneakers, at Rockefeller Center, and the slot renders at most 416 CSS px wide, which puts the **face at 60px tall**. Three candidates were rendered at exactly 416 x 520 and looked at, which is the only way to judge this: full figure 60px, **head to mid thigh 97px, chosen**, head to hands 118px. Face height at a fixed render width depends only on how much of the frame the face occupies, so **cropping vertically alone buys nothing** and the crop has to come in from the sides too; it is centred on his body at x=1720 rather than on the frame, because he stands right of centre. It costs the legs, the sneakers and the gilded Prometheus statue, which sits at x 0 to 680 and cannot survive any crop tight enough to fix the face, and it keeps the plaza's flags, the hedges, the brass railing and the whole shirt print. The tightest candidate was rejected because at 118px the face is barely better than 97 and the shirt print, which is most of the photograph's character, gets cut in half. Output is 1000 x 1250 WebP at quality 86, 153 KB; **none of iteration 14's or 60's corner work applies**, this being a rectangular photograph whose radius is CSS. **The cap moved from 20rem to 26rem and the ratio did not**, which is the re-derivation the old note demanded rather than a deletion of it: the cap's job is no longer sharpness but stopping a 4:5 photograph from out-growing the headline, 416px sits within 24px of the 392px track at 1512 so the track governs where it matters, and widening the ratio would push the track past the cap and restore the dead sand the ratio exists to prevent. **The old asset was the soft one all along**: `headshot.jpg` measured **0.81 at every viewport from 390 to 3840**, 318 CSS px rendered needing 636 against 512 available, and the note's "renders at 1.6x" had simply left DPR out. The photograph measures 1.21 to 1.59, so it is the first image this slot has held that is not upscaled. Alt text rewritten, the old one having described a gray hoodie and a cream wall. Verified across nine viewports from 360 to 3840 at DPR 2: the hero grid at 1512 is 669px tall against a 489px portrait, so the text column still sets the row height, and at 2560 the portrait leads by 17px. Audit ALL CLEAN. Not changed: **`index.html` still uses `headshot.jpg` for the 48px round bulb in the hero eyebrow**, which is what a tight square face crop is for, so that asset stays live; a bulb derived from this photograph would make the two pages agree on which photograph of him the site uses, and that is worth doing, but it is a change to the home page and this request was about the about page.
iterations/63-keybank-year-and-legacy-screens/  KeyBank's home page card back to **2019**, and the **two legacy screens moved out of section 05 Experience into section 01 Background**, asked for together: *"Make Keybank's year 2019 and then the two top images in the experience section should go in the background"*. The year reverts the one row of iteration 62 that was not read off its own page: `keybank.html` says March 2018 twice and its one dated quote is April 2018, but **a six month UX delivery from March 2018 hands off to a build the page does not cover**, and the person who was there knows which year the project is filed under, so the page not mentioning 2019 is a gap in the page rather than evidence against the card. The span's comment now says so and tells the next reader not to correct it again, because the mismatch will keep looking like a defect to anyone who only reads the repo. **The two screens were misfiled and their own captions proved it**: "This is the 55 minutes" points at the first sentence of the Background prose and "This is the crash in the quote above" points at the Background pull quote, both about 2,000px further up the page, so the evidence for the problem sat inside the section showing the solution and a reader met the old system only after reading the whole new flow. Background now reads as claim, testimony, evidence; section 05 is the designed flow only, in flow order, ten screens opening on Branch information. Neither caption was edited, "The same legacy system" in the second alt still refers to the figure directly above it, and section 05's "the way the old system did" still reads because the old system has been shown by then. Markup is a plain `<div class="shots">` in `.case-section__body`, so the one-up grid, `--shots-gap` and the `--s-6` top margin all come for free with **no new CSS**; deliberately **not** `.shots-group`, which widens the paragraph above to `--measure-case-wide` and raises its size, right for a lead-in and wrong for body copy. Both sections sit in the same `.container.case-section__grid`, so the images render at exactly the width they did before, which is what makes this a move and not a resize. Verified at 1512 and 390, DPR 2: two `.shot` figures in `#background` and ten in 05, no broken images, no overflow, one h1, no console errors, audit ALL CLEAN. Notes a pre-existing defect this did not cause and does not fix: **every one of the twelve KeyBank desktop screens measures 0.96 natural-to-rendered-times-DPR at 1512**, 1600px sources against a rendered 833 x 2, which wants a re-export rather than anything in this change.
iterations/62-project-card-years/  the five home page project cards' years changed from START years to RANGES read off each case study's own Timeline cell, prompted by *"The year in the project thumbnail - is it a problem? ... there have been comments around it being a little old. What does data or the internet say about that?"*. **The research half is answered by saying there is no answer**: no web search tool in this session, `nngroup.com/articles/ux-portfolio/` is a 404 and its site search returns nav and footer only, and no credible public dataset measures how a screener reads a date on a portfolio card. The six-second figures that circulate are **resume** eye-tracking, and stretching them here would be inventing a finding. What is defensible without a citation: screeners read for relevance, scope and recent comparable work, an 18 to 24 month old enterprise fintech flagship is unremarkable at senior level, and what reads badly is a portfolio with nothing recent, which is not this one. **The real defect was in the repo, not in the perception.** The cards carried start years, which is the least flattering number available for anything that ran long, and two of the five read as contradictions of the page they link to: Transactions said 2022 against a Timeline of "October 2023, September 2024, September 2025", and KeyBank said 2019 when `keybank.html` says March 2018 twice and carries an April 2018 quote. Now: Pay Bill **2022-23** (to the October 2023 launch), Card Dashboard **2025-26** (to the July 2026 launch), Basket Building **2026** unchanged, Transactions **2022-25**, KeyBank **2018** (reverted to 2019 the same day, see iteration 63). A range also does a job a single year cannot, which is to show how long the programme ran, and duration is scope: Pay Bill now reads as 18 months rather than as a 2022 project. **KeyBank got one year older and that is the honest cost** of the card agreeing with its own page, the same defect as Transactions' 2022 pointing the other way; fixing one and leaving the other would have been picking the flattering half. **Transactions is the one card whose first year is not read off its page, deliberately**: 2022 was asked for explicitly in iteration 58, replacing a 2024 that had shipped, so that value has already been corrected once by the person who knows the kickoff date; the Timeline's three dates are releases rather than a kickoff, so a 2022 start is consistent with them where 2022 *alone* next to a page whose earliest date is 2023 was not. If 2022 was the date of the screens in the thumbnail instead, it becomes 2023-25 and nothing else moves. `.project__year` gained **`white-space: nowrap`**, because an en dash is a break opportunity and at 390 the chip broke after it and read as two lines, `2022-` over `23`; the row has no `flex-wrap`, so the cost lands on `.project__tag`, which already wrapped to two lines at that width and takes a third at 320. Verified at DPR 2 across 1512, 1280, 1024, 860, 744, 430, 390 and 320: the chip is one line at every width, 14px tall against 29 before the `nowrap`, the gap to the tag never falls below `--s-3`, nothing clips, no document overflow. Not fixed: the home page is still ordered by strength rather than chronology, 2022-23, 2025-26, 2026, 2022-25, 2018, which I still think is right for a trailer now that the dates are legible.
iterations/61-prototype-fits-the-viewport/  the Basket Building live prototype sized by the VIEWPORT rather than by the embed's width, asked for as *"I like how it looks in 32 inch monitor ... For in 16 inch macbook pro, the prototype looks too big ... Should take up the most space of the viewport"*. The phone frame's zoom stepped on embed width alone and its top band is the 1.35 cap, so **every viewport wide enough to give the embed 760px got the same 1259px of embed height**: 1.02 of the room under the nav on a 2560x1320 monitor, which is why that one looked right, but 1.37 on a 16 inch MacBook Pro and 1.61 on a 14 inch. **The prototype could not fix this itself**, and the reason is worth keeping: section 8a fits the iframe to the prototype's content height, so framed, the prototype's own viewport height IS its content height and a `max-height` query inside that file would only ever read its own answer back. The host is the only thing that knows the room, so the host solves the zoom and hands it down. The bands now set `--phone-zoom` and one rule reads `zoom: var(--proto-zoom, var(--phone-zoom, 1))`, so a host that sets `--proto-zoom` wins and **the file opened on its own is unchanged**, verified at 1.35 and 0.80. `zoomToFit()` in site.js solves `room = (innerHeight - nav.bottom) * FILL - surround`, then `z = min(room / phoneNat, (embedWidth - 96) / 393, 1.35)` clamped up to 0.62. **`FILL = 1` is a calibration, not a taste judgement**: the monitor the user liked was already at 1.02, so holding one screenful holds that screen and fixes the rest. `nav.bottom` comes out because `.nav` is fixed and owns the top 80px, and reading it at any scroll is safe because `is-stuck` only changes its background. `surround` is the heading, intro and two selects, which do not zoom, measured live because it grows when the selects wrap; `phoneNat` is read back through the zoom in force so the band that picked it does not matter, and the frame is found by which one has a box so the treatment on show does not either. **The 96px gutter is not `.proto-wrap`'s 20px of padding**: allowing only the padding pushed a 390px viewport to 0.79 and nearly edge to edge, and back-solving the bands gives 340 - 0.62x393 = 96px of clear width at their narrowest, so 96 reproduces the 0.62 band exactly at 390 and stops binding above about 560px of embed. One-shot solve, not an iteration, because `surround` does not depend on the zoom, which is also why it cannot fight the height fit: the zoom is computed from the viewport and the embed's width, never from the height `fit()` writes. A `resize` listener joins the existing `ResizeObserver` because **a resize that changes only the window's height leaves the embed's width alone**, so the embedded body never reflows and never reports. After: fill 1.00 at 744, 860, 1280, 1512, 1728 and 1920, and **1.33 at 2560 against the 1.35 it had**, a 1.5% move, which is the point. 390 stays width-capped at 0.62. Two dead ends recorded there: targeting a share of `innerHeight` for the embed ignores the 190px of `.proto-block` chrome above it, and targeting a share of the room under the nav for the PHONE pushes the selects up under the nav so the visitor cannot see the control and the phone at once. The `--proto-h` bands in tokens.css stay as the no-JS and cross-origin fallback and their note now says so.
iterations/60-bundle-carousel-hero/  the Basket Building hero image replaced with the current Figma frame, node `2373:29042` of `H3gOVBp2rmrTas26UAY195`, asked for as *"In improving basket building hero, show this phone frame/image"*. `bundle-carousel.webp` at 940x1937 retires `treatment-single.webp` at 840x1787, which had no other user on any live page; the wrapper, the `--bare` treatment and the no-caption decision are untouched. **Same composition, later screen, and the difference is the subject of the page**: one bundle card becomes a scrollable row of them, and the card gains **three ticked reasons the bundle fits** under a "Summarized from product information" line with the Alexa+ mark. Those reasons are what the AI Builder work is for, so the hero had been showing an earlier screen than the page describes. **The alpha is iteration 14's problem by a different route and iteration 14's fix does not work.** A Figma frame export has no alpha, so the four corners came back opaque white, which on `--bg` is the four white nicks the user flagged three times; but this bezel is **not a circular-cornered rectangle**, it is a continuous-curvature squircle. The proof is two readings that no circle can satisfy at once: the first non-white pixel down the left edge is at x=161 when y=0 and at x=0 when y=170, and a circle of radius R centred at (R,R) requires R=161 and R=170 respectively. So `proc.py` **reads the mask off the image instead of drawing it on**: the body is convex and the background is exactly white, so per row it is everything between the leftmost and rightmost non-white pixel, which is exact whatever the curve is; and each boundary pixel's own value gives its coverage, `a = (255-v)/(255-b)` with `b` from three pixels further in, so it gets alpha `a` and RGB `b`. That second step is why **there is no pale hairline to chase**: iteration 14 had to paint the discarded wedge `#1A1A1A` so LANCZOS would not drag white back along the curve, and here every partial pixel is already bezel-coloured with its whiteness moved into the alpha channel. Transparent pixels still get RGB zeroed, for iteration 11's reason. `TRIM = 1` drops the outermost ring because the node is 571.139 tall, so at scale 4 the last row is 56% covered and arrived `(158,158,156)`, a pale line along the bottom of a dark phone that the row-wise read does not model. **940 wide, not the 840 the other four phones use**, because this is the only one sized off `--case-hero-phone-h`, `clamp(26rem, 64vh, 60rem)`: at the 60rem ceiling the phone is asked for 960 CSS px of height, 1920 device px at DPR 2, which at this aspect is 932 wide, so 840 would have upscaled on a large monitor. Measured ratio of natural width to rendered width times DPR: 2.33 at 390, 1.63 at 744, 1.68 at 1024, 1.89 at 1280, 1.54 at 1512, 1.40 at 1920 and **1.05 at 2560**, the row that settles it. 138 KB against the old 101 KB, which is what that last row costs. Two things left open and recorded there: **`home-card.webp` still shows the old three treatments**, iteration 22 having baked it from iteration 14's crops, so the home page thumbnail and this hero now show different generations of the same screen; and the ticked reasons **name the Sonos app while the bundle is an LG soundbar**, which is in the Figma frame rather than the encode and is legible from 1512 up. **Amended the same day for a re-export of the same node**, asked for as *"Update that image again with this"* after the frame was edited in Figma: same encoder, same filename, same 940x1937, so no HTML changed. The diff between the two 1109x2285 exports has bbox `(48,46,1060,337)`, so nothing below y=337 moved; the status bar and the "Search Amazon" field both dropped about 40 source pixels, 10 CSS px, which is effectively extra top padding in the teal header. 137,062 bytes against the first export's 138,524, corners and all seven viewport ratios re-verified unchanged. That nudge adds a third open item: **the search field now clips the top of the "prime One-Day" line** it used to clear, again in the frame rather than the encode. The superseded source is `source/superseded/figma-2373-29042-4x-first-export.png` and the superseded WebP is `before/bundle-carousel-first-export.webp`.
...
2. ~~Real headshot.~~ Done: `assets/img/headshot.jpg`, 512x512, cropped tight on
   the face. Since iteration 64 its **only** user is the 6rem thermometer bulb on
   the home page hero eyebrow pill (see section 7), the about page hero having moved
   to `assets/img/about/portrait.webp`, so any replacement must still be a square
   crop, at least 400x400, with the face centered and no important detail in the
   corners. The two pages therefore show two different photographs of him, which is
   noted as open in `iterations/64-about-portrait-photo/`.
...
6. ~~`about.html`. The M.S. HCI degree belongs there; it is nowhere on the site yet.~~
   ...
   transcript. The portrait asset question is **closed**: the hero photograph is now
   `assets/img/about/portrait.webp`, 1000 x 1250, cropped from a 3300 x 4397
   Rockefeller Center photograph the user supplied, and `assets/img/headshot.jpg` is
   retained only for the 48px round bulb on the home page hero eyebrow. See
   `iterations/64-about-portrait-photo/` rather than item 3 of
   `iterations/42-about-page/README.md`, which describes the search that failed.
...
- **`.about-portrait` in the hero media slot.** The one hero media on the site that
  ...
  `.case-hero__device` is sized for a phone bezel. It is `assets/img/about/portrait.webp`,
  1000 x 1250, and it is capped at **26rem** rather than filling its track. The cap
  used to exist for sharpness, because the only portrait asset was a 512px square
  headshot that a 509px 1fr track rendered at 1:1; that reason died with iteration
  64, and what the cap does now is stop a 4:5 photograph from growing taller than
  the headline beside it. 26rem is 416px against a 392px track at 1512, so the
  track governs from 1024 to 1512 and the photo fills it, and the cap only bites
  at 744 stacked and above 1920. The grid ratio shifts to 1.9fr/1fr, the same track
  the bare phone hero takes, so the photo is not marooned in the middle of a wide
  column, and **that ratio did not move when the cap did**: widening it would push
  the track past the cap and put the dead sand back. If the cap moves again, check
  it against the 1512 track the same way. The old note claimed the 512px headshot
  rendered at 1.6x, which left DPR out: measured, it was 0.81 at every viewport
  from 390 to 3840, so it was the one soft image on the page the whole time.
